...
pub async fn run_pipe(
    ...
    let pipe_dir = cubby_dir.join("pipes").join(pipe);
    ...
    env_vars.push((
        "cubby_DIR".to_string(),
        cubby_dir.to_str().unwrap().to_string(),
    ));
...
pub async fn download_pipe(source: &str, cubby_dir: PathBuf) -> anyhow::Result<PathBuf> {
    info!("Processing pipe from source: {}", source);

    let pipe_name = sanitize_pipe_name(Path::new(source).to_str().unwrap());

    let dest_dir = cubby_dir.join("pipes").join(&pipe_name);
