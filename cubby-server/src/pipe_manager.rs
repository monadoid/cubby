use anyhow::Result;
use cubby_core::{download_pipe, download_pipe_private, PipeState};
...
impl PipeManager {
    pub fn new(cubby_dir: PathBuf) -> Self {
        PipeManager {
            cubby_dir,
    ...
    pub async fn update_config(&self, id: &str, new_config: Value) -> Result<()> {
        ...
        let pipe_dir = self.cubby_dir.join("pipes").join(id);
    ...
    pub async fn list_pipes(&self) -> Vec<PipeInfo> {
        let pipe_dir = self.cubby_dir.join("pipes");
    ...
    pub async fn download_pipe(&self, url: &str) -> Result<String> {
        ...
        let pipe_dir = download_pipe(&normalized_url, self.cubby_dir.clone()).await?;
    ...
    pub async fn download_pipe_private(
        ...
        let pipe_dir = download_pipe_private(pipe_name, url, self.cubby_dir.clone()).await?;
    ...
    pub async fn stop_pipe(&self, id: &str) -> Result<()> {
        ...
        if let Some(handle) = pipes.remove(id) {
            ...
            cubby_core::pipes::cleanup_pipe_crons(id).await?;
            ...
            #[cfg(windows)]
            {
                ...
                let _ = tokio::process::Command::new("powershell")
                    ...
                    .arg("-Command")
                    .arg(format!(
                        r#"Get-WmiObject Win32_Process | Where-Object {{ $_.CommandLine -like "*.cubby\pipes\{}*" }} | ForEach-Object {{ taskkill.exe /T /F /PID $_.ProcessId }}"#,
                        &id.to_string()
                    ))
    ...
    pub async fn start_pipe_task(&self, id: String) -> Result<impl Future<Output = Result<()>>> {
        let cubby_dir = self.cubby_dir.clone();
        let running_pipes = self.running_pipes.clone();
        let id_for_map = id.clone();

        Ok(async move {
            match cubby_core::run_pipe(&id, cubby_dir.clone()).await {
