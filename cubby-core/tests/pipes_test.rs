...
mod tests {
    ...
    async fn test_download_pipe_invalid_url() {
        ...
        let cubby_dir = temp_dir.path().to_path_buf();

        let invalid_url = "https://example.com/invalid/url";
        let result = download_pipe(invalid_url, cubby_dir.clone()).await;
    ...
    async fn test_nextjs_pipe_app_dir() {
        ...
        let pipe_task = tokio::spawn(run_pipe("nextjs-test-pipe", cubby_dir.clone()));
    ...
    async fn test_sanitize_pipe_name() {
        ...
        let github_url_subdir = "https://github.com/monadoid/cubby-sp/tree/main/pipes/search";
    ...
    async fn test_downloading_and_running_sideloaded_pipe() {
        ...
        let dest_path = cubby_dir.join("pipes").join(pipe_name);
        ...
        let urls = vec![
            ...
            "https://github.com/monadoid/cubby-sp/tree/main/pipes/search",
        ];

        for url in urls {
            let result = download_pipe(url, cubby_dir.clone()).await;
            ...
            // run pipe
            let run_result = run_pipe(&pipe_name, cubby_dir.clone()).await;
    ...
    async fn test_downloading_and_running_private_pipe() {
        ...
        let result = download_pipe_private("data-table", source, cubby_dir.clone()).await;
        ...
        assert!(cubby_dir.join("pipes").join(pipe_name).exists(),
            "test failed for downloading private pipe: {:?}",
            result.err()
        );

        // any zip shouldn't exists
        let mut entries = tokio::fs::read_dir(cubby_dir.join("pipes").join(pipe_name)).await.unwrap();
        ...
        // verify pipe.json
        let pipe_json_path = cubby_dir.join("pipes").join(pipe_name).join("pipe.json");
        let pipe_json_content = tokio::fs::read_to_string(&pipe_json_path).await.unwrap();
        let pipe_json: serde_json::Value = serde_json::from_str(&pipe_json_content).expect("Invalid JSON format");
        ...
        let pipe_dir = cubby_dir.join("pipes").join(pipe_name);
        let pipe_json_path = pipe_dir.join("pipe.json");
        let pipe_json = tokio::fs::read_to_string(&pipe_json_path).await.unwrap();
        let mut pipe_config: serde_json::Value = serde_json::from_str(&pipe_json).unwrap();
        pipe_config["enabled"] = json!(true);
