...
pub async fn trigger_and_check_screen_recording() -> Result<bool> {
    ...
    for attempt in 1..=MAX_ATTEMPTS {
        if CGPreflightScreenCaptureAccess() {
            tracing::info!("Screen recording permission granted after {} seconds", attempt);
            return Ok(true);
        }
    ...
    tracing::warn!("Screen recording permission not granted after {} seconds", MAX_ATTEMPTS);
    Ok(false)
}
...
pub async fn trigger_and_check_microphone() -> Result<bool> {
    ...
    tracing::warn!("Microphone permission not granted after {} seconds", MAX_ATTEMPTS);
    Ok(false)
}
