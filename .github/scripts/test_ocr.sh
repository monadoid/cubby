#!/bin/bash
convert -size 300x100 xc:white -font DejaVu-Sans -pointsize 24 -fill black -draw "text 10,50 'Hello, cubby OCR'" test_image.png
...
do
   sleep 10
   ps -p $(cat cubby.pid) -o %cpu,%mem,cmd
done
kill $DISPLAY_PID

# Extract OCR text from SQLite
OCR_TEXT=$(sqlite3 $HOME/.cubby/db.sqlite "SELECT text FROM ocr_text;")

# Check if OCR detected the expected text
if echo "$OCR_TEXT" | grep -qi "Hello, cubby OCR"; then
