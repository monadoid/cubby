...
impl From<cubby_db::AudioEntry> for AudioEntry {
    fn from(db_entry: cubby_db::AudioEntry) -> Self {
        Self {
...
impl FrameCache {
    pub async fn new(cubby_dir: PathBuf, db: Arc<DatabaseManager>) -> Result<Self> {
        let cache_config = CacheConfig {
            cache_dir: cache_dir().unwrap().join("cubby").join("frames"),
            ..Default::default()
        };
