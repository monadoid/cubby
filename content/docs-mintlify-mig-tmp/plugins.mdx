...
cubby is built for extensibility through plugins that interact with captured screen and audio data. whether you need to tag activities, generate summaries, or send data to third-party services, plugins let you build powerful workflows.

plugins run within cubby's sandboxed environment. written in typescript/javascript and nextjs.
...
regardless of progress in AI architecture, it's as good as the given context. cubby is the bridge between dull hallunicating AI and super intelligent agents.
...
- **no lock-in**: use our store for distribution, then drive traffic into your exported standalone app
  - [cubby-tauri-template](https://github.com/LorenzoBloedow/cubby-tauri-template-dev)
  - [cubby-electron-template](https://github.com/neo773/cubby-electron)

#### killer features

- **ai flexibility**: OpenAI, local LLMs (ollama), Anthropic, Gemini, etc.
- **rich APIs**:
  - `pipe.querycubby` for context
...
```bash
bunx --bun @cubby/dev@latest pipe create # or use npx @cubby/dev pipe create
bun dev
```
...
![developer account](https://raw.githubusercontent.com/monadoid/cubby/main/content/developer-account.png)
...
bunx --bun @cubby/dev@latest login

# create a new pipe
bunx --bun @cubby/dev@latest pipe register --name my-pipe [--paid --price 9.99] # MAKE SURE TO SET THE PRICE IF YOU WANT TO SELL IT - IRREVERSIBLE
# you'll receive payouts to your stripe account on a daily basis

# publish your pipe to the store
bunx --bun @cubby/dev@latest pipe publish --name my-pipe
...
bunx --bun @cubby/dev@latest pipe list-versions --name my-pipe
...
# end current session
bunx --bun @cubby/dev@latest logout
```

you can deploy your pipe to your cubby app through the UI or using `cubby pipe install <path>` and `cubby pipe enable <id/folder of your pipe>`.

when you're ready to deploy, send a PR to the [cubby repo](https://github.com/monadoid/cubby) to add your pipe to the store.
...
- https://github.com/monadoid/cubby/blob/main/pipes/obsidian/src/app/api/log/route.ts
- https://github.com/monadoid/cubby/blob/main/pipes/search/src/components/search-chat.tsx
- https://github.com/monadoid/cubby/blob/main/pipes/rewind/src/lib/hooks/use-app-name-suggestion.tsx
- https://github.com/monadoid/cubby/blob/main/cubby-server/src/server.rs
- https://github.com/monadoid/cubby/blob/main/cubby-core/src/pipes.rs
