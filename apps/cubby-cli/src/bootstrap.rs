...
pub const SCREENPIPE_BINARY: &str = if cfg!(target_os = "windows") { "screenpipe.exe" } else { "screenpipe" };
pub const CLOUDFLARED_BINARY: &str = if cfg!(target_os = "windows") { "cloudflared.exe" } else { "cloudflared" };
...
pub fn ensure_binaries() -> Result<BinPaths> {
    ...
    let screenpipe = extract_binary("screenpipe", &temp_dir)
        .context("Failed to extract screenpipe binary")?;
    let cloudflared = extract_binary("cloudflared", &temp_dir)
        .context("Failed to extract cloudflared binary")?;
