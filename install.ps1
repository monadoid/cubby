...
try {
    ...
    $installDir = "$env:USERPROFILE\cubby"
    $tempZip = "$env:TEMP\cubby.zip"
    ...
    # Verify installation
    $binPath = Join-Path $installDir "bin\cubby.exe"
    if (!(Test-Path $binPath)) {
        throw "cubby.exe not found in $binPath after installation"
    }
