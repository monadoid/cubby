mod tests {
    ...
    async fn test_audio_speaker_identification() {
        ...
        if multiple_speakers_sample_rate != 16000 {
            multiple_speakers_data = cubby_audio::resample(
        ...
        let (mut obama_data, obama_sample_rate) =
            cubby_audio::pcm_decode(&obama_path).expect("Failed to decode audio file");

        // reesample if not 16000
        if obama_sample_rate != 16000 {
            obama_data = cubby_audio::resample(&obama_data, obama_sample_rate, 16000).unwrap();
        }
