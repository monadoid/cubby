...
cubby offers robust capabilities for integrating with various MCP clients, allowing users to leverage its features across different platforms. This guide will walk you through the setup and usage of cubby with supported MCP clients.
...
Directory: C:\Users\divan\.cubby\mcp
Config file: C:\Users\divan\.cubby\mcp\config.json

To run the MCP server, use this command:
$ uv --directory C:\Users\divan\.cubby\mcp run cubby-mcp --port 3030
```

## Configuration

The configuration for the MCP server is stored in `config.json` located in the `.cubby\mcp` directory. Here is an example configuration:
...
cubby integrates with **Claude Desktop**, allowing seamless interaction via MCP. Follow this guide to set it up properly.
...
![Claude Setup Gi](https://media.githubusercontent.com/media/monadoid/cubby/refs/heads/main/content/docs-mintlify-mig-tmp/public/claude-setup.gif)

If the setup is successful, you'll see a **hammer icon** appear in the interface.

## Cursor IDE

cubby MCP can be integrated with **Cursor IDE** to make it available across all your projects. Follow these steps to set it up.
...
- Click the **refresh icon** next to MCP servers.
  ...
  <Frame>
    ![Cursor IDE](https://media.githubusercontent.com/media/monadoid/cubby/refs/heads/main/content/docs-mintlify-mig-tmp/public/cursor-setup.gif)
  </Frame>
...
For more detailed information, please refer to our [GitHub repository](https://github.com/monadoid/cubby/tree/main/cubby-integrations/cubby-mcp).
