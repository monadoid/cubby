...
use cubby_server::video_cache::FrameCache;
...
async fn setup_test_env() -> Result<(FrameCache, Arc<DatabaseManager>)> {
    ...
    let cubby_dir = home_dir()
        .expect("couldn't find home dir")
        .join(".cubby")
        .join("data");

    debug!("using real cubby data dir: {:?}", cubby_dir);
    ...
    let cache = FrameCache::new(cubby_dir, db.clone()).await?;
    Ok((cache, db))
}
