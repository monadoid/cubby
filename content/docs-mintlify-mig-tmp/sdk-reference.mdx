...
cubby provides two sdk packages:

- `@cubby/js` - for node.js environments (nextjs api routes, etc)
- `@cubby/browser` - for browser environments

both sdks provide type-safe interfaces to interact with cubby's core functionality.

[feel free to use our docs as context in cursor agent through MCP](https://cubby.sh/docs/mcp-server#mintlify-mcp-use-our-docs-in-cursor%2C-claude%2C-etc)
...
<Tabs>
  <Tab title="npm">
    ```bash
    npm install @cubby/browser
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash
    pnpm add @cubby/browser
    ```
  </Tab>
  <Tab title="bun">
    ```bash
    bun add @cubby/browser
    ```
  </Tab>
  <Tab title="yarn">
    ```bash
    yarn add @cubby/browser
    ```
  </Tab>
</Tabs>
...
const results = await pipe.querycubby({
  ...
  browserUrl: "github.com/monadoid/cubby" // Filter by browser URL
})
```
...
check how the obsidian pipe implements this, [route](https://github.com/monadoid/cubby/blob/main/pipes/obsidian/src/app/api/log/route.ts) and [pipe.json](https://github.com/monadoid/cubby/blob/main/pipes/obsidian/pipe.json) for a complete example.
...
```bash
bunx @cubby/dev@latest components add
...
⚠️ make sure to enable real time transcription in the cubby app settings or CLI args.
...
pipes can access and modify cubby app settings through the SDK. this is useful for storing pipe-specific configuration and accessing global app settings.
...
- `get-cubby-app-settings` server action
...
```typescript
// src/lib/types.ts
import type { Settings as cubbyAppSettings } from '@cubby/js'
...
export interface Settings {
  ...
  // cubby app settings
  cubbyAppSettings?: cubbyAppSettings
}
```
...
```typescript
// src/lib/actions/get-cubby-app-settings.ts
import { pipe } from '@cubby/js'
import type { Settings as cubbyAppSettings } from '@cubby/js'

export async function getcubbyAppSettings() {
  return await pipe.settings.getAll()
}

export async function updatecubbyAppSettings(
  newSettings: Partial<cubbyAppSettings>
) {
...
import {
  getcubbyAppSettings,
  updatecubbyAppSettings,
} from '@/lib/actions/get-cubby-app-settings'
...
export function usePipeSettings() {
  ...
  const loadSettings = async () => {
    try {
      // load cubby app settings
      const cubbySettings = await getcubbyAppSettings()

      // get pipe specific settings from customSettings
      const pipeSettings = {
        ...(cubbySettings.customSettings?.yourPipeName && {
          ...cubbySettings.customSettings.yourPipeName,
        }),
      }
      ...
      setSettings({
        ...DEFAULT_SETTINGS,
        ...pipeSettings,
        cubbyAppSettings: cubbySettings,
      })
    } catch (error) {
  ...
  const updateSettings = async (newSettings: Partial<Settings>) => {
    try {
      // split settings
      const { cubbyAppSettings, ...pipeSettings } = newSettings
      ...
      // update cubby settings if provided
      await updatecubbyAppSettings({
        ...cubbyAppSettings,
        customSettings: {
          ...cubbyAppSettings?.customSettings,
      ...
      setSettings({
        ...mergedPipeSettings,
        cubbyAppSettings:
          cubbyAppSettings || settings?.cubbyAppSettings,
      })
      return true
    } catch (error) {
...
- store pipe-specific settings under `customSettings.yourPipeName` in cubby app settings
...
see the [obsidian pipe](https://github.com/monadoid/cubby/tree/main/pipes/obsidian) for a complete example of settings management.

### integrating with llms

cubby's sdk can be easily integrated with various ai providers to analyze and generate insights from screen activity. here are common patterns for connecting context data with llms.

if the user is using cubby-cloud, you can use claude-3-7-sonnet, gemini-2.0-flash-lite or gpt-4o (Anthropic, Google, OpenAI or local models).
...
async function generateWorkLog(screenData, model, startTime, endTime) {
  ...
  const prompt = `You are analyzing screen recording data from cubby.
...
async function handleRequest() {
  ...
  // query context from cubby
  const screenData = await pipe.querycubby({
...
see the [obsidian pipe](https://github.com/monadoid/cubby/tree/main/pipes/obsidian) for the complete implementation.
...
import { ollama } from "ollama-ai-provider";
import { ContentItem } from "@cubby/js";
...
import { pipe } from "@cubby/js";
import { OpenAI } from "openai";
import type { Settings } from "@cubby/js";


async function streamMeetingInsights(settings: Settings) {
  const openai = new OpenAI({
    apiKey:
      settings?.aiProviderType === "cubby-cloud"
        ? settings?.user?.token
  ...
  for await (const chunk of pipe.streamTranscriptions()) {
    ...
    if (transcript.length > 200 && transcript.length % 30 === 0) {
      const response = await openai.chat.completions.create({
        model: "gpt-4o", // if user is using cubby-cloud, you can also use claude-3-7-sonnet or gemini-2.0-flash-lite
        messages: [
          {
            role: "system",
...
check out our [production pipe examples](https://github.com/monadoid/cubby/tree/main/pipes) on github to see more ai integration patterns.
...
- https://github.com/monadoid/cubby/blob/main/cubby-js/browser-sdk/src/index.ts
- https://github.com/monadoid/cubby/blob/main/cubby-js/node-sdk/src/index.ts
- https://github.com/monadoid/cubby/blob/main/cubby-js/common/Operator.ts
