...
async fn create_test_video() -> Result<PathBuf> {
    let cubby_dir = dirs::home_dir()
        .expect("couldn't find home dir")
        .join(".cubby")
        .join("data");

    info!("looking for monitor video in {}", cubby_dir.display());

    // Read directory and find first video containing "monitor"
    let mut entries = tokio::fs::read_dir(&cubby_dir).await?;
...
async fn test_get_video_metadata() -> Result<()> {
    ...
    let metadata =
        cubby_server::video_utils::get_video_metadata(video_path.to_str().unwrap()).await?;
