...
let traversalQueue = DispatchQueue(label: "com.cubby.traversal")

// Add cubbyDB instance
var cubbyDb: cubbyDB?
...
let synchronizationQueue = DispatchQueue(label: "com.cubby.synchronization")
...
func getStateFilePath() -> String {
    ...
    let appSupportDir = paths[0].appendingPathComponent("cubby")
...
func setupDatabase() {
    ...
    do {
        ...
        if sqlite3_exec(cubbyDb?.db, createTableSQL, nil, nil, nil) != SQLITE_OK {
            let error = String(cString: sqlite3_errmsg(cubbyDb?.db))
        ...
        if sqlite3_prepare_v2(cubbyDb?.db, addColumnSQL, -1, &stmt, nil) == SQLITE_OK {
        ...
        if !columnExists {
            ...
            if sqlite3_exec(cubbyDb?.db, alterTableSQL, nil, nil, nil) != SQLITE_OK {
                let error = String(cString: sqlite3_errmsg(cubbyDb?.db))
...
func saveElementValues() {
    ...
    sqlite3_exec(cubbyDb?.db, "BEGIN TRANSACTION", nil, nil, nil)
    ...
    sqlite3_exec(cubbyDb?.db, "COMMIT", nil, nil, nil)
...
// Get the universal cubby database path
func getcubbyDbPath() -> String {
    let homeDir = FileManager.default.homeDirectoryForCurrentUser
    return homeDir.appendingPathComponent(".cubby/db.sqlite").path
}
