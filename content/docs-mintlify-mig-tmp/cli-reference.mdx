...
below is the detailed cli reference for cubby's core functionality.
...
- **ocr-engine** (`\-o, --ocr-engine <ENGINE>`): OCR engine selection
  - options:
    ...
    - `custom`: configurable via `cubby_CUSTOM_OCR_CONFIG`
...
# 2. configure cubby
export cubby_CUSTOM_OCR_CONFIG='{
  "api_url": "http://localhost:8000/ocr",
  "api_key": "",
  "timeout_ms": 5000
}'

# 3. run cubby with custom ocr
cubby add \
  path/to/videos \
  --ocr-engine custom \
  --data-dir /tmp/sp
...
```bash
# list all pipes
cubby pipe list [--output <FORMAT>] [--port <PORT>]

# install a new pipe
cubby pipe install <URL> [--output <FORMAT>] [--port <PORT>]

# get pipe info
cubby pipe info <ID> [--output <FORMAT>] [--port <PORT>]

# enable/disable pipe
cubby pipe enable <ID> [--port <PORT>]
cubby pipe disable <ID> [--port <PORT>]

# delete pipe
cubby pipe delete <ID> [-y] [--port <PORT>]

# purge all pipes
cubby pipe purge [-y] [--port <PORT>]
```

#### add external data to cubby (OCR only)

allows you to add external screen recordings to cubby, for example it could be your iphone screen recordings, your physical journal photos, etc.

companies use this to index their product's screen recordings, for example.

```bash
# add video files
cubby add <PATH> [--data-dir <DIR>] [--output <FORMAT>] [--pattern <REGEX>] [--ocr-engine <ENGINE>] [--metadata-override <PATH>]
```

by default, cubby extracts metadata (fps, duration, creation time) directly from video files. however, you can override these with a metadata file:
...
find $HOME/.cubby/data -type f -name "*monitor*.mp4" | head -n 3 | while read -r file; do
    case $RANDOM in
        [0-9][0-9][0-9][0-9])
...
# 3. run cubby with metadata override
cubby add \
...
note: if you don't provide a metadata override file, cubby will automatically extract metadata from the video files. use overrides when you need to specify custom metadata or when the automatic extraction fails.
...
The `cubby` CLI supports generating shell completions for popular shells. Follow the steps below to enable autocompletion for your shell:
...
1. Create `\~/.config/fish/conf.d/cubby.fish` and add this line to it:

```fish
cubby completions fish | source
```
...
```powershell
cubby completions powershell | Out-String | Invoke-Expression
```
...
- https://github.com/monadoid/cubby/blob/main/cubby-server/src/bin/cubby-server.rs
- https://github.com/monadoid/cubby/blob/main/cubby-server/src/cli.rs
