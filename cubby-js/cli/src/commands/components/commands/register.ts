    ...
    if (error)
      if (error instanceof Error) {
        if (error.message === "No such file or directory") {
          logger.break()
          logger.warn("this command can only be called from within the cubby-js/cli of cubby's repository");
          process.exit(1)
        }
      }
    logger.break()
...
export const registerComponentCommand = new Command()
  .name("register")
  .description("register a new component in cubby's component registry")
  ...
  .action(async (opts) => {
    try {
      ...
      logger.log("run `bun run build` and open a PR at https://github.com/monadoid/cubby-sp to update registry.");
    } catch (error) {
      logger.break();
      handleError(error);
    }
