...
import { SearchToolInputSchema, SearchResponseSchema } from '../schemas/screenpipe'
...
export function generateMcpOpenAPISpec(baseUrl: string): OpenAPISpec {
    const spec: OpenAPISpec = {
        ...
        info: {
            ...
            version: '1.0.0',
            description: 'Model Context Protocol (MCP) tools for accessing Screenpipe data. These tools are accessed via JSON-RPC 2.0 at the /mcp endpoint, but are documented here in OpenAPI format for convenience.'
        },
        servers: [
            {
                url: baseUrl,
...
function getMcpTools() {
    // Use the actual schemas from our screenpipe definitions
    ...
    const DocumentedSearchInputSchema = SearchToolInputSchema.extend({
        ...
        content_type: z.enum(['all', 'ocr', 'audio', 'ui', 'audio+ui', 'ocr+ui', 'audio+ocr']).optional().describe('Type of content to search for'),
        start_time: z.string().datetime().optional().describe('Start time for search range (ISO 8601)'),
        end_time: z.string().datetime().optional().describe('End time for search range (ISO 8601)'),
        ...
        include_frames: z.boolean().optional().describe('Include frame data in results'),
        min_length: z.number().optional().describe('Minimum content length'),
        max_length: z.number().optional().describe('Maximum content length'),
        speaker_ids: z.array(z.number()).optional().describe('Filter by speaker IDs (for audio content)'),
    ...
    return [
        {
            name: 'search',
            description: 'Search screen and audio content on a user\'s Screenpipe device. Supports filtering by text query, content type (ocr/audio/ui), time range, application, and more. Returns structured results with OCR text, audio transcriptions, and UI element captures.',
