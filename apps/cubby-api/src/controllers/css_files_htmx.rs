    ...
    if response.status().is_success() {
        ...
        if let Ok(json_array) = serde_json::from_str::<Vec<Value>>(&content) {
            ...
            for json in &json_array {
                ...
                let contains = json
                    .get("http://www.w3.org/ns/ldp#contains")
                    ...
                    for item in contains {
                        if let Some(full_url) = item.get("@id").and_then(|id| id.as_str()) {
                            ...
                            let is_directory = item
                                ...
                                .map(|types| {
                                    types.iter().any(|t| {
                                        t.as_str() == Some("http://www.w3.org/ns/ldp#Container")
                                            || t.as_str() == Some("ldp:Container")
                                    })
                                })
                                .unwrap_or(false);
...
pub async fn upload(
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|e| Error::string(&format!("Multipart error: {}", e)))?
    {
        if field.name() == Some("file") {
            ...
            let response = auth_client
                .authenticated_binary_request("PUT", &path, Some(&headers), Some(data.to_vec()))
                .await
    ...
    Ok(Response::builder()
        ...
        .header(
            "HX-Redirect",
            format!("/files?path={}", urlencoding::encode(&path)),
        )
...
pub async fn view(
    ...
    let response = client
        .authenticated_request("GET", &path, None, None)
        .await
...
pub async fn delete(
    ...
    let response = client
        .authenticated_request("DELETE", &path, None, None)
        .await
