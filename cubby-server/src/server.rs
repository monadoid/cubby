...
use cubby_vision::monitor::{get_monitor_by_id, list_monitors};
...
pub async fn health_check(State(state): State<Arc<AppState>>) -> JsonResponse<HealthCheckResponse> {
    ...
    for device in &audio_devices {
        ...
        let last_capture = cubby_audio::core::get_device_capture_time(&device_name);
    ...
    if audio_devices.is_empty() {
        let last_capture = cubby_audio::core::LAST_AUDIO_CAPTURE.load(Ordering::Relaxed);
...
impl SCServer {
    ...
    pub async fn create_router(&self, enable_frame_cache: bool) -> Router {
        let app_state = Arc::new(AppState {
            ...
            cubby_dir: self.cubby_dir.clone(),
            ...
            frame_cache: if enable_frame_cache {
                Some(Arc::new(
                    FrameCache::new(self.cubby_dir.clone().join("data"), self.db.clone())
                        .await
                        .unwrap(),
                ))
            } else {
...
async fn merge_frames_handler(
    ...
    let output_dir = state.cubby_dir.join("videos");
    ...
    match payload.content.content_type.as_str() {
        "frames" => {
            if let ContentData::Frames(frames) = &payload.content.data {
                if !frames.is_empty() {
                    let output_dir = state.cubby_dir.join("data");
                    let time = Utc::now();
...
//         device: cubby_core::DeviceType::Audio(device.clone()),
//         is_running: true,
//         is_paused: false,
//     };
...
async fn semantic_search_handler(
    ...
    State(state): State<Arc<AppState>>,
) -> Result<JsonResponse<Vec<cubby_db::OCRResult>>, (StatusCode, JsonResponse<Value>)> {
...
//         .device_manager
//         .update_device(DeviceControl {
//             device: cubby_core::DeviceType::Vision(payload.device_id),
//             is_running: true,
//             is_paused: false,
//         })
//         .await;
    ...
    let incoming = tokio::spawn(async move {
        while let Some(Ok(msg)) = receiver.next().await {
            if let Message::Text(t) = msg {
                if let Ok(event) = serde_json::from_str::<cubbyEvent>(&t) {
...
async fn get_pipe_build_status(
    ...
    let pipe_dir = state.cubby_dir.join("pipes").join(&pipe_id);
