...
use cubby_audio::transcription::stt::SAMPLE_RATE;
...
async fn test_transcription_accuracy() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            r#"bro - got some good stuff from cubby here's the lowdown on your day, you productivity ninja: absolutely demolished that 2-hour coding sesh on the new feature. the keyboard is still smoking, bro! crushed 3 client calls like a boss. they're probably writing love letters to you as we speak, make sure to close john tomorrow 8.00 am according to our notes, let the cash flow in! spent 45 mins on slack. 90% memes, 10% actual work. perfectly balanced, as all things should bewatched a rust tutorial. way to flex those brain muscles, you nerd! overall, you're killing it! 80% of your time on high-value tasks. the other 20%? probably spent admiring your own reflection, you handsome devil. ps: seriously, quit tiktok. your fbi agent is getting bored watching you scroll endlessly. what's the plan for tomorrow? more coding? more memes? world domination? generated by your cubby ai assistant (who's definitely not planning to take over the world... yet)"#,
        ),
        (
    ...
    for (audio_file, expected_transcription) in test_cases {
        ...
        let task = tokio::spawn(async move {
            ...
            let audio_data =
                cubby_audio::pcm_decode(&audio_path).expect("Failed to decode audio file");
