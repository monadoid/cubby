...
export function generateMcpOpenAPISpec(baseUrl: string): OpenAPISpec {
  const spec: OpenAPISpec = {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "Model Context Protocol (MCP) tools for accessing Screenpipe data. These tools are accessed via JSON-RPC 2.0 at the /mcp endpoint, but are documented here in OpenAPI format for convenience.",
    },
    servers: [
      {
        url: baseUrl,
  ...
  return [
    {
      name: "search",
      description:
        "Search screen and audio content on a user's Screenpipe device. Supports filtering by text query, content type (ocr/audio/ui), time range, application, and more. Returns structured results with OCR text, audio transcriptions, and UI element captures.",
