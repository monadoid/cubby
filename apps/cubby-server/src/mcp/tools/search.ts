 ...
 * MCP Search Tool
 *
 * Provides AI assistants with the ability to search Screenpipe content
...
mcpServer.tool('search', {
    description: 'Search screen and audio content on a user\'s Screenpipe device. Supports filtering by text query, content type (ocr/audio/ui), time range, application, and more.',
    ...
    handler: async (args: SearchToolInput, ctx: MCPServerContext) => {
        ...
        console.log(`MCP search tool proxying to device ${deviceId}${path} (request ID: ${requestId})`)
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`Device proxy error: ${response.status} ${response.statusText} - ${errorText}`)
                throw new Error(`Device request failed: ${response.status} ${response.statusText}`)
            }
            ...
            throw new Error(`Failed to search device: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
})
